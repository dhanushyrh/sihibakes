...
import { AdminFeatureTogglesSection } from "@/components/admin/settings/AdminFeatureTogglesSection";
...
export function AdminSettingsPageClient({
  ...
  const startShopEdit = () => {
    ...
    setShopDraft({
      ...
      free_delivery_km: settings.free_delivery_km ?? 5,
    });
  ...
  const saveShopDelivery = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        free_delivery_km: shopDraft.free_delivery_km,
        updated_at: new Date().toISOString(),
      })
      .eq("id", settings.id);
    setSavingSection(null);
  ...
  return (
    <div>
      ...
      <AdminFeatureTogglesSection
        settings={settings}
        whatsappConfigured={whatsappConfigured}
        adminAlertPhoneConfigured={adminAlertPhoneConfigured}
        ...
        {editingShop ? (
          <>
            <div className="mt-4 space-y-4">
              ...
              <div>
                <p className="text-xs font-medium text-[#4B2C20]/60">
                  Free delivery within (km)
                </p>
                <p className="mt-0.5 text-xs text-[#4B2C20]/45">
                  Orders within this distance from the kitchen get ₹0 delivery
                  fee. Set to 0 to disable.
                </p>
                <input
                  type="number"
                  min={0}
                  step={0.5}
                  value={shopDraft.free_delivery_km}
                  onChange={(e) =>
                    setShopDraft((prev) => ({
                      ...prev,
                      free_delivery_km: Number(e.target.value),
                    }))
                  }
                  className="mt-2 w-full max-w-xs rounded-xl border px-3 py-2 text-sm sm:w-auto"
                />
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              />
              <ViewField
                label="Free delivery within"
                value={
                  (settings.free_delivery_km ?? 0) > 0
                    ? `${settings.free_delivery_km} km`
                    : "Disabled"
                }
              />
            </div>
